...
public class Woolc
{
    public static enum Phase {PARSE, SEMANTIC, COMPILE};
...
//                        + "\n#\n########################################\n");
    ...
    private void processProgram(StringBuilder woolText) throws Exception
    {
        runner = WoolFactory.makeParserRunner(CharStreams.fromString(woolText.toString()));
        switch (phase) {
            case PARSE: runner.parse(); break;
            case SEMANTIC: runner.semantic(); break;
//            case IR: runner.makeIR(); break;
    ...
    private void writeOutput() throws Exception
    {
        if (bytecode != null) {
        	for (String s : bytecode.keySet()) {
                String classFilePath =  outputDirectory + "/wool/" + s;
    ...
    private void postProcess(StringBuilder woolText)
    {
        if (displaySource) {
            System.out.println("\n------------------------------\nSource:\n");
            System.out.println(woolText.toString());
        }
...
//        }
    ...
    private void showHelp()
    {
        System.out.println("Usage: coolc [ options ] [ sourcefiles ]\n"
            ...
            + "    -o outputdirectory\n"
            + "    -h\n"
            + "    -p phase (parse|semantic)\n"
            + "    -d internals (pt|gui|tables|source)");
    }
